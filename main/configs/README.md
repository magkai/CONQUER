...
In the following, the used parameters for training and evaluation configurations are explained. Precomputed data is used for efficiency. Also see the provided sample configs for the main experiments.
...
|entropy_const | 0.1 | Factor for entropy regularization |
...
|num_epochs | 10 | Number of epochs model should be trained (checkpoints are stored after each epoch) |
...
|negative_reward |    [true, false] | Whether reward of -1 should be used in case of an reformulation (compared to 0) |
|observation_spec_shape_x |  [1001,1002] | Observation shape (first direction): Concatenation of question (+history) + action embeddings (1002 if history is used) |
|observation_spec_shape_y |   769 |  Observation shape (second direction): Concatenation of observation and action mask  |
|checkpoint_path |  -  | Storing location for model checkpoints  |
| pretrained |  [true, false] | Whether a pre-trained model should be loaded from which training will be continued|
|pretrained_path |  -  | Location from which pretrained model should be loaded  |
...
|next_info_predictions | - | When noisy reformulation predictor is used, predictions for each question and its follow up info need  |
|ref_predictions |   - |  When noisy reformulation predictor is used: predictions for each question and its next reformulation  |
|next_info_fraction |   - | If fractional reward should be used: use probabilities from reformulation predictor directly (likelihood for different info need) |
|ref_fraction | - | If fractional reward should be used: use probabilities from reformulation predictor directly (likelihood for same info need)  |
...
|encoded_history |   - | Location of encoded conversation history if used, default: null|
...
| agg_type  |  ["add", "max", "majo", "maxmajo"] | Aggregation type for final ranking (default: "add") |
|observation_spec_shape_x |  [1001,1002] | Observation shape (first direction): Concatenation of question (+history) + action embeddings (1002 if history is used)  |
|observation_spec_shape_y |   769 |  Observation shape (second direction): Concatenation of observation and action mask  |
|filename |   -  | Path were results should be stored  |
|checkpoint_path |  -  | Checkpoint location of trained model |
|checkpoint_nbr | 10 | Initial learning rate for adam optimizer |
...
|bert_history |  - | Location of encoded conversation history if used, default: null|
